...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-white shadow-md py-4 px-6 flex items-center justify-between">
      ...
      {/* Sign out button (visible only on /dashboard) */}
      {isDashboard && (
        
        <button
          onClick={handleLogout}
          className="text-sm px-4 py-1 rounded cursor-pointer hidden md:inline-block"
          style={{ color: "#367AFF" }}
        >
          Sign out
        </button>
